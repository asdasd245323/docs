...
## [Prerequisites](/api/guides/core/events/handling-data-change-notifications#prerequisites)
...
- Familiarity with configuring [webhooks](/api/guides/core/events/events-webhooks)
...
## [Guide](/api/guides/core/events/handling-data-change-notifications#guide)
...
### [Port-out credentials](/api/guides/core/events/handling-data-change-notifications#port-out-credentials)
...
### [User](/api/guides/core/events/handling-data-change-notifications#user)

To handle changes to user information, register a webhook endpoint for the [`user.updated`](/api/events/schemas/com.gigs.user.updated) event. This event is emitted for changes to the [`User`](/api/core/users#the-user-resource) resource.
...
### [Subscription](/api/guides/core/events/handling-data-change-notifications#subscription)

Every subscription has a plan. To handle a plan change for a subscription, register a webhook endpoint for the [`subscriptionChange.applied`](/api/events/schemas/com.gigs.subscriptionChange.applied) event. It may occur to you to use [`subscription.updated`](/api/events/schemas/com.gigs.subscription.updated) instead since a plan is a nested resource on a [`Subscription`](/api/core/subscriptions#the-subscription-resource). But then we would have to probe `previousData` to understand that it was a plan change. The more specific an event, the simpler our code!
...
Now would be the time to bring the good news to the user. But first, we have to look them up. The remainder of our approach here is similar to our handling of [`portOutCredentials.issued:`](/api/guides/core/events/handling-data-change-notifications#port-out-credentials)
...
| Event | Description |
| --- | --- |
| [`subscription.created`](/api/events/schemas/com.gigs.subscription.created) | Represents the initial point in the subscription lifecycle. |
| [`subscription.activated`](/api/events/schemas/com.gigs.subscription.activated) | Indicates the subscription is active and the user has connectivity. |
| [`subscription.restricted`](/api/events/schemas/com.gigs.subscription.restricted) | Indicates the subscription is restricted and the user has limited connectivity. |
| [`subscription.restored`](/api/events/schemas/com.gigs.subscription.restored) | Indicates the subscription is restored (back to active state) and the user has full connectivity again. |
| [`subscription.renewed`](/api/events/schemas/com.gigs.subscription.renewed) | Indicates a successfully renewed subscription, marking the start of a new service period. |
| [`subscription.canceled`](/api/events/schemas/com.gigs.subscription.canceled) | Indicates subscription cancellation. |
| [`subscription.resumed`](/api/events/schemas/com.gigs.subscription.resumed) | Indicates the subscription is no longer canceled. |
| [`subscription.ended`](/api/events/schemas/com.gigs.subscription.ended) | Represents the end of the subscription lifecycle. |
...
## [Final Thoughts](/api/guides/core/events/handling-data-change-notifications#final-thoughts)
...
If would like to browse all events published by Gigs, check out our [events schema](/api/events/schemas/com.gigs.apiKey.created).
